...
| **Persistent render knobs** — `KEY=value` per line, sourced by `render-lab-config.sh` and re-used by `update.sh`, so an upgrade re-renders the same config (`HARDWARE_ARMED`, `TRACKER_YAW_DEG`, `LOG_LEVEL`, `CAMERA_SERIALS`, `DORA_BIND_HOST`). Not re-derivable — back it up | `$DATA_ROOT/lab.env` | `mavis-v2:mavis-v2`, 664 |
...
What this machine keeps there: `HARDWARE_ARMED=true`, `MIC_ENABLED=true`,
...
| **Render knobs** — NOT re-derivable: without it a re-render silently drops `HARDWARE_ARMED`, the tracker yaw, `CAMERA_SERIALS`, `DORA_BIND_HOST` | `$DATA_ROOT/lab.env` |
...
#    要持久保留的渲染开关写进 ~/apollo-mavis-v2-ws/var/lab.env（HARDWARE_ARMED、TRACKER_YAW_DEG、
