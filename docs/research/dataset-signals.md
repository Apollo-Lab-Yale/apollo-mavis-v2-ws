...
| Firmware / SN / DH | `arm.version`, `get_robot_sn()`, `get_dh_params()` (fw >= 2.0.0), `get_servo_version()`, `get_gripper_version()`, `get_linear_track_version/_sn()` | once | str | – | – | L (metadata) | Not recorded anywhere yet (phase-09 todo). Every high-value signal here is firmware-gated. |
