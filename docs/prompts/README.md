...
- **phase-15（GELLO Manipulation，2026-09-09 插入；契约 `docs/design/16-gello.md` v1.0）**：landing 页**第五张卡片 GELLO Manipulation**
  （wire `mode: gello` + `SessionSpec.gello {viewpoint: auto|external|hold}`）。被动的 xArm7 形 **GELLO leader 臂**（Dynamixel 舵机、一只
  FTDI FT232H 串口适配器；runtime 设备 `devices/gello.py` `GelloReader`，与 Vive controller 同一模式，新 extra `[gello]`）在**关节空间只驱动
  Manipulation Arm**（七个关节 + 夹爪跟随 trigger；键盘 ←/→ 仍驱动其导轨），经同一条统一步长 cap + 孪生门禁——leader 再快 follower 也只以 cap
  跟随、绝不跳变；**Perception Arm 由外部 viewpoint 节点**经既有 dora 外部接口驱动（`SessionAnnounce.external_arms: ["view"]`——末尾追加的
  新字段，`action_names` 只有 view 块；两仓 contract golden 同步），无人发布时停在 GELLO hold 位姿。**厨房孪生 `mavis_v2_kitchen`**
  （`hidden: true`，`mavis_v2` 仍是唯一暴露场景）= 细胞 + 2026-09-09 用 Perception Arm 腕部 D435i 实测的冰箱 GE GDE21ESKSS / GE 30" 电炉灶 /
  台面 / 吊柜 box 与四块 tagStandard41h12 贴图板（电器面 ±3 cm，待 `*_align` 叠加与真机相机核对后再改 YAML）。**启动 = 检查 → 规划 →
  逐臂执行 → 接合**：读 leader、±2π 关节展开到离实测最近的分支、关节限位 + 孪生碰撞检查（409 含碰撞对；无 session 的 `POST /api/gello/preview`
  返回判定 + 碰撞体红色高亮 PNG，sheet 每 500 ms 轮询、只有 `clear` 才能 Start），双臂在厨房孪生上规划并由 `_execute_arms` **一次一臂**执行，
  到位后按 tolerance 接合；接合状态机 `no_leader | out_of_sync | tracking | paused | motion`，只有 `tracking` 跟随、其余 hold 上一条命令；
  `R` / Go to profile / 退出回位先强制 `paused`，Pause / Resume 是 Cockpit 按钮（键表 24 行不改）。标定为无 session 的
  `POST /api/gello/calibrate {op: match_arm|gripper_open|gripper_closed|clear}`（偏置取 π/2 整数倍写 `var/gello_calibration.json`，
  `joint_signs` 是 operator 配置）。**真机放开 gello**（D8；`hardware_session.armed` 照旧把关，Online DAgger 真机仍 409）。v1 不录制。
  附带修复 Cockpit **clearance 读数**（用户第 6 条）：runtime 仍发 5 对，UI 只显示最近 4 对、单行省略号、`max-height` 自滚动，
  EpisodeControls 移到读数之上（16-gello §12.4）。部署：runtime `uv sync … --extra gello`（dynamixel-sdk + pyserial）、udev `0403:6014` →
  `dialout` + ftdi_sio `latency_timer` 1 ms（lab 机已有规则文件，需 `FORCE_UDEV=1` 重跑）、render knobs `GELLO_BACKEND` / `GELLO_USB_SERIAL` /
  `GELLO_BAUD` / `TWIN_OVERLAY_SCENE`（DEPLOYMENT S4 / S5 / S7 / S8.1c / S11）。依赖 phase-12 + 13 + 14——三者及 2026-09-08/09 追加项已于
  **2026-09-09 05:46 提交推送（ws e16d2c1 钉住五仓）**，上文 phase-12 / 13 / 14 各条里"未提交"的状态描述到此为止。2026-09-09 适配器已枚举但
  舵机总线无应答（疑舵机未通电，16-gello §16 第 1 条）；首次真机验收待用户在场（`phase-15-gello.md` 验收标准）。
