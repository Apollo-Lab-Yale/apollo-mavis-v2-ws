...
- [ ] **mavis_v2 场景校准**（`apollo-xarm7-sim/.../scenes/mavis_v2.yaml` 头部有全部
      假设）：① 实物轨道/滑块宽度——口述障碍物纵深 27.5 起、贴 gripper 轨内侧，按
      mavis 网格（轨 19.2 cm、滑块凸出 1.8 cm）会重叠 3 cm，孪生里暂放纵深 23.8 起，
      实测后回调；② 轨道零位端悬出长度（网格按基座右侧距右沿 14 cm 会伸出台面 4.5 cm）；
      ③ "14 cm"量到基座还是滑块；④ 两轨 base 间距 39.5 与相机轨外沿 2.6 复核；
      ⑤ 臂法兰安装面高出台面 0.107188 m 是否属实；⑥ 电机盒在哪一端。
