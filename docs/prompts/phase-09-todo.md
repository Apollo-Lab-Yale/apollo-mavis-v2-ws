...
- [ ] **mavis_v2 场景校准**（`apollo-mavis-v2-sim/.../scenes/mavis_v2.yaml` 头部有全部
      ...
      **39.0 cm**（grip y0 −0.1786），卷尺的 39.5 在其误差内。**两臂原先都离障碍
      ...
      真机 1.075 m（x0 = 0.2800 后画出来的轨止于台面 −X 边缘前 0.24 cm——早先 x0 = 0.2750 时曾写作
      越过 0.26 cm；故意把零位端对准——那一端有障碍物）；
      ⑥ **腕相机外参已于 2026-09-06 实测**：`wrist_cam` 由参考模型的猜测 (0.07, 0, 0.05)
      改为 (0.06832, −0.02220, 0.02945)（残差回归移动 `base_pos` 后重解；首解曾为 (0.07294,
      −0.01709, 0.03019)），横向约 22 mm、离法兰近约 20 mm——这就是叠加图对不齐的
