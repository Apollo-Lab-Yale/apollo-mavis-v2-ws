...
- [ ] **mavis_v2 场景校准**（`apollo-xarm7-sim/.../scenes/mavis_v2.yaml` 头部有全部
      假设）：① 轨道零位（SDK 0 mm）确在台面**右端**、行程朝左，电机盒在哪一端；
      ② "14.5 cm 到右沿"量的是基座中心（当前 x0=0.4625）还是基座边/滑块边
      （0.3995/0.3645）——按网格，轨道右端会伸出台面 10 cm，若实物齐平说明是后者；
      ③ 两轨间距 39.5 cm 与"gripper 轨到前沿 42 cm"差 5 mm，以实测为准；④ 实物
      轨道宽度（mavis 网格 19.2 cm 含 3 mm 线槽板，本体 14.2）决定两轨间通道宽度
      （当前 20.3 cm，箱子两侧仅余约 1.3 cm）；⑤ 箱子是否紧贴某条轨道（"紧卡"）、
      底边 16 cm（口述曾说 13）；⑥ 臂法兰安装面高出台面 0.107188 m 是否属实。
