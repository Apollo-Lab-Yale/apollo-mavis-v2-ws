...
- [ ] **mavis_v2 场景校准**（`apollo-xarm7-sim/.../scenes/mavis_v2.yaml` 头部有全部
      假设）：① 轨道零位（SDK 0 mm）确在台面**右端**、行程朝左；② "14.5 cm 到右沿"
      量的是滑块边（当前取值 x0=0.3645）还是基座边/基座中心（0.3995/0.4625）；
      ③ 两轨外沿 2 cm / 42 cm 与"轨间 39.5 cm"差 5 mm，以实测为准；④ 实物轨道
      宽度（mavis 网格 19.2 cm 含 3 mm 线槽板）与右端是否与台面齐平；⑤ 障碍物
