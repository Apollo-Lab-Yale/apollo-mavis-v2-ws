...
1. **BRINGUP** — instantiate the workcell for the requested `kind` (POST
   ...
   **Hardware bring-up (phase-09c/09d, `SessionManager._bringup_hardware` over
   ...
   collection admitted on hardware 2026-09-07 (§10.5); DAgger / inference —
   POLICY-DRIVEN motion — admitted behind `hardware_session.policy_modes` since
   2026-09-12 (operator decision; repo default `false`, the lab render sets it with
   `HARDWARE_POLICY_MODES=true`; 15-online-dagger D7 as amended). With the knob false
   the matrix answers 409 `"hardware sessions support teleop and data collection only
   (<mode> on hardware: not yet - hardware_session.policy_modes is false; the lab
   render sets it with HARDWARE_POLICY_MODES=true)"` FIRST, before anything else — an
   Online DAgger body (`mode: dagger` + `policy_source: external` + `online_dagger`,
   §10.7) is refused by the same line before its own checks run. With it true the
   policy is resolved INSIDE the matrix, before a box is enabled: a checkpoint through
   `resolve_policy` (`"unknown policy …"` / `"no promoted deploy checkpoint …"`), an
   external policy through the shared `"no external policy attached (…)"` (an Online
   DAgger body leaves that to `_check_online_dagger` so its documented 409 order
   holds); a `dagger` body additionally needs a live wrist camera (`"DAgger rollout
   recording needs at least one live hardware camera - none is open …"`), and step 11
   below builds the policy stack instead of the plain loop. `home_rail`, the arming
   switch, the monitor hand-over and every other line of the matrix are untouched.**
   Since
   ...
   11. *`MinkIKSolver` (`lock_rail = not control.rail_in_ik`), `SceneKinematics`,
       ...
       *2026-09-12, `mode ∈ {dagger, inference}` behind `hardware_session.policy_modes`
       (step 11c in the code)* — the plain loop is REPLACED: `_build_policy_stack(spec,
       rig.session_cfg, rig.workcell, rig.twin.scene, session_id, loop.ik, loop.kin,
       rig.twin, rig.supervisor, control_cfg=loop.cfg, loop_kwargs={workcell_kind:
       "hardware", gripper_arms, tracker, speed_scale}, recorder_kwargs={kind:
       "hardware", cameras: the adopted previews, camera_cfgs})` returns the
       `GatedPolicyExecutor` (+ the rollout recorder for dagger, + the policy session),
       built over the SAME capped control config, gate, twin, IK / kinematics and tracker
       provider the plain loop had (§11); the never-started plain `ControlLoop` is
       dropped and the executor takes its place in the rig, so `start_from`,
       return-to-start, the gate-held abort, the fault path, teardown and the telemetry
       see ONE loop exactly as in sim; a dagger body without a live wrist camera is 409
       here as collect is. Then `loop.start()` → recorder → policy session, every start
       inside the rollback scope (a failure stops them in reverse, abandons a fresh Online
       DAgger directory, then the rig's D6 hand-back).
...
- **Refusals** (`_check_online_dagger`, evaluated at `create()` after
  `_check_dataset_spec` and the hardware refusal matrix — hardware + dagger is 409
  there while `hardware_session.policy_modes` is false, admitted with it true since
  2026-09-12, §5 — and BEFORE `_check_return_to_start`, before any side effect; order
...
- **Hardware (D7, amended 2026-09-12)**: admitted behind `hardware_session.policy_modes`
  (§5 step 11, §11; the lab render has it ON since 2026-09-12) but NOT yet exercised on
  the real cell / the lab dora plane — sim + the runtime's fake trainer node
  (`dora_bridge/nodes/fake_policy.py`, `FAKE_TRAINER=1`: capability `online_dagger`,
  `preparing` → `ready`, trains after `FAKE_TRAINER_EVERY` 2 kept rollouts or on
  `train_now`, bumps the version, knobs `FAKE_TRAINER_PREPARE_S` 0.5 /
  `FAKE_TRAINER_TRAIN_S` 1.0 / `FAKE_TRAINER_FAIL_AT`) and the fake-driver seam
  (`tests/test_hardware_session.py::test_online_dagger_session_is_admitted_with_policy_modes_and_records_rollouts`)
  only. First run on the real arms: speed scale 10 %, E-stop in hand, a recorded episode
  replayed through the playback dialog (§10.8) BEFORE any policy drives. Numbers,
  deviations and open items: 15-online-dagger §12.
...
- **admission**: sim sessions, and hardware sessions only with
  `hardware_session.policy_modes` (`"action replay is admitted in sim only
  (hardware_session.policy_modes is false)"` — the per-tick gate is the only check it
  gets and the twin does not model the room, 03-sim §4.5; the operator admitted it on
  hardware on 2026-09-12 behind the knob, D7 as amended — it is the FIRST policy-class
  motion to try on the real arms, at 10 %), teleop / collect sessions only
...
- **Hardware (operator decision 2026-09-12; `hardware_session.policy_modes`).** The
  same stack runs on the real cell. `_bringup_hardware` (§5 step 11) hands the rig's
  pieces to `_build_policy_stack` through three keyword groups: `control_cfg` = the
  speed-scaled, servo-capped `ControlConfig` the plain hardware loop was built with
  (`scale_control_config` → `executor_caps_for` → `apply_executor_caps` /
  `apply_teleop_caps`), `loop_kwargs` = `workcell_kind="hardware"`, the config's gripper
  arms, the rig's tracker provider and speed scale (merged over the sim defaults by
  `_policy_loop_kwargs`; sim callers pass nothing), `recorder_kwargs` = `kind="hardware"`
  + the ADOPTED wrist cameras and their intrinsics for `_build_collect_recorder` — so the
  `GatedPolicyExecutor` (checkpoint or `ExternalPolicySource`, with or without the
  `OnlineDaggerCoordinator`) is built over the unconditional `SafetyGate` on the gate
  twin, the twin-bound `MinkIKSolver` / `SceneKinematics` / planner, and a dagger
  rollout is recorded from the same live cameras a collect session uses (both wrist
  videos, twin-camera extrinsics, `online_dagger/<s>/rollouts`). Unchanged on purpose:
  the executor's `SlewLimits` and the anchor leash come from the unscaled `cfg.control`
  (as in sim); the driver-side servo caps carry the session's speed scale (10 / 50 /
  100 %) exactly as for teleop. Teardown is the generic order (policy session → recorder
  → loop → drivers' D6 hand-back → previews' fps → monitor). Fake-driver tests:
  `tests/test_hardware_session.py` (`test_external_inference_session_is_admitted_with_policy_modes`,
  `test_online_dagger_session_is_admitted_with_policy_modes_and_records_rollouts`). NOT
  yet run on the real arms — first trial = a recorded episode through the playback
  dialog (§10.8) at 10 %.
...
On the real cell (2026-09-12) inference is admitted behind `hardware_session.policy_modes`
exactly like dagger (§5 step 11, §11): the executor runs over the rig's capped control
config and the unconditional gate, recorder OFF, Space / `takeover` the escape; not yet
exercised on the real arms.
...
`set_collision_sensitivity` maintenance op (0..3, 0 = OFF; §13.1), read from the
...
hardware_session:            # phase-09c/09d (§5 hardware BRINGUP, §13.1 home_rail)
  ...
  policy_modes: false        # POLICY-DRIVEN MOTION on the real arms (operator decision 2026-09-12):
                             #   inference / dagger sessions (§5 step 11, §11 / §12) and the
                             #   action-column playback (§10.8) are admitted on hardware only when
                             #   true; repo default false, the lab render sets it with
                             #   HARDWARE_POLICY_MODES=true (DEPLOYMENT.md S4; ON in the lab since
                             #   2026-09-12). The per-tick twin gate, the leash and the step caps
                             #   are the safety authority either way; NOT yet exercised live
...
`set_collision_sensitivity` maintenance op (§13.1) writes a level 0..3 (0 = detection OFF) on top
...
| Operator lowers / restores the collision sensitivity (Cockpit sensitivity control in a hardware session, Hardware-tab arm card without one; 2026-09-11 — the fridge door's seal force reads as C31 at sensitivity 3) | `POST /api/hardware/arms/{arm_id}/maintenance {op: set_collision_sensitivity, collision_sensitivity: 0..3}` | session: `request_set_collision_sensitivity` on the driver's monitor thread, result awaited ≤ 10 s via `setting_result()` (ok on code 0 or a 1 / 2 / 9 status echo, the note in `warnings`); no session: the read-only monitor's poll thread writes once and judges by the rich-frame read-back (§13.1). Neither moves the arm; the driver's phase / budget / streamer are untouched. The level is remembered per arm (telemetry `arms[*].collision_sensitivity`, the monitor row while paused, `backstops_match`) until the next driver connect re-applies the config value — including 0, which is falsy and must NOT be read as "nothing requested"; 422 outside 0..3, 409 while a rail homing runs |
