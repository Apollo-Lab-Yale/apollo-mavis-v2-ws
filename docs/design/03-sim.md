...
- **The Perception Arm's posture here is also a runtime PROFILE**, `Kitchen Interaction`
  (operator request 2026-09-09 evening; `python -m
  apollo_mavis_v2_runtime.profiles.seed_kitchen`, 04-runtime §10.5), so it can be reached
  on the real cell with "Go to profile" now that the mode it was built for is gone. That
  profile pins the **Perception Arm's** carriage, because the appliance numbers below only
  line up with the real cameras from this carriage position. It does NOT carry this
  keyframe's Manipulation Arm: since 2026-09-10 (operator request) the grip entry is the
  DEFAULT posture with its carriage unset, so a goto moves the Perception Arm only. The
  keyframe's `+π` joint 1 and the default's `−π` are the same orientation but differ by a
  full 360° of joint travel, which the planner used to walk before every kitchen session.
