...
(cd "$WS/apollo-mavis-v2-runtime" && uv build --wheel --python 3.12 -o "$WHEELS" "$SRC")
...
(cd "$WS/apollo-mavis-v2-runtime" && uv pip install --no-deps --force-reinstall "$WHEEL")
(cd "$WS/apollo-mavis-v2-runtime" && uv run python -c "import pysurvive, sys; print('pysurvive OK from', pysurvive.__file__)")
